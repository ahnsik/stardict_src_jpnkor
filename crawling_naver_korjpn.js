...
fs.writeFileSync("kjdic.txt", ' \n', 'utf8');   // to Clear latest file.
for (var i=start_index; i<start_index+98163; i++) {     // 시스템(서버)의 용량에 따라 다르겠지만, 내 PC 에서는 한번에 300개 정도가 최대 값이다. 그 이상 되면 먹통이 되더라..
  ...
  (function(i) {     // 이것은 단지 i 라는 값을 callback 내에서 알맞게 쓰기 위한 방법일 뿐.
    ...
    try {
      ...
      // request('GET', url, function(error, response, html) {
        ...
        fs.appendFileSync("kjdic.txt", title_str + "\t" + meaning_blk + '\n', 'utf8');
