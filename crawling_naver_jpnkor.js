...
var start_index=67848;  // 일한사전 (JK) : i<=303350  (total:  0 ~ 303350)
                      // 한일사전 (KJ) : i<=98162  (total:  0 ~ 98162)
                      // 한자어 사전에 대해서는 아직 no-idea.
for (var i=start_index; i<start_index+30; i++) {     // 시스템(서버)의 용량에 따라 다르겠지만, 내 PC 에서는 한번에 300개 정도가 최대 값이다. 그 이상 되면 먹통이 되더라..
      // {  var i=67848;
  ...
  request(url, function(error, response, html) {
    ...
    meaning_blk = temp_blk.replace(/\n/g, "");
      ...
      /* 추가로 해야 할 일 들..
        1. 중복된 SPACE 들을 정리. ==> <h5>                접미사       </h5>  이렇게 쓸데 없이 많은 SPACE 정리.  (C코드로 정리했음)
        2. "lst mean_level_1 kr_lst ""   <-- 여기 끝에 쌍따옴표 닫는거 이상함. 이것도 고칠 것.      정상인 경우 안나옴.
